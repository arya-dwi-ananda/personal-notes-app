...
class App extends React.Component {
  ...
  render() {
    ...
    const filteredNotes = notes.filter((note) => note.title.toLowerCase().includes(searchKeyword.toLocaleLowerCase()) || note.body.toLowerCase().includes(searchKeyword.toLocaleLowerCase()));
    ...
    return (
      <div className="note-app" data-testid="note-app">
        <div className="note-app__header" data-testid="note-app-header">
          <h1>Notes</h1>
          <div data-tesid="note-search">
            <input data-testid="note-search-input" type="text" placeholder="Cari catatan" value={searchKeyword} onChange={(event) => this.onSearchHandler(event.target.value)} />
          </div>
        </div>
